...
export default {
  ...
  computed: {
    filteredItems() {
      ...
      const allItems = [...this.items, ...this.userPostedItems, ...this.featuredItems, ...this.clickedItems];
  ...
  methods: {
    ...
    fetchUserPostedProducts() {
      axios.get(`${API_BASE_URL}user-products`)
        .then(response => {
    ...
    submitRating(item) {
      axios.post(`${API_BASE_URL}products/${item.id}/rate`, { rating: item.user_rating })
        .then(response => {
